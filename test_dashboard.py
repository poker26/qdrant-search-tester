...
def get_collection_schema(collection: str):
    """Определяет имена векторов в коллекции."""
    try:
        ...
        if isinstance(vectors, dict):
            if "dense" in vectors:
                ...
                # первый попавшийся dense вектор
                for name in vectors:
                    dense_name = name
                    break
        ...
        return {"dense": dense_name, "sparse_names": sparse_names}
    except Exception:
        return {"dense": "dense", "sparse_names": ["sparse"]}
...
def do_search(collection: str, emb: EmbeddingResult, mode: str, limit: int, score_threshold: float):
    ...
    Автоматически определяет имена векторов из схемы коллекции.
    """
    schema = get_collection_schema(collection)
    dense_name = schema["dense"] or "dense"
    sparse_names = schema["sparse_names"]

    # Для sparse/hybrid: если коллекция имеет "sparse" — используем его,
    # иначе берём первый доступный sparse вектор (напр. "ingredients")
    sparse_name = "sparse" if "sparse" in sparse_names else (sparse_names[0] if sparse_names else None)
    ...
    elif mode == "sparse" and emb.sparse and sparse_name:
        resp = client.query_points(
            collection_name=collection,
    ...
    elif mode == "hybrid" and emb.sparse and sparse_name:
        ...
        if not embedder:
            ...
            with st.spinner("Поиск..."):
                ...
                with c1:
                    ...
                    if rd:
                        st.dataframe(results_to_df(rd), use_container_width=True, hide_index=True)
                    else:
                        st.warning("Пусто")
                with c2:
                    st.subheader(f"Sparse ({ts:.2f}с)")
                    if rs:
                        st.dataframe(results_to_df(rs), use_container_width=True, hide_index=True)
                    else:
                        st.warning("Пусто / нет sparse")
                with c3:
                    st.subheader(f"Hybrid RRF ({th:.2f}с)")
                    if rh:
                        st.dataframe(results_to_df(rh), use_container_width=True, hide_index=True)
                    else:
                        st.warning("Пусто / нет sparse")
